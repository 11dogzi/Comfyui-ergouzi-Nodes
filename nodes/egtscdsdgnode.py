...
class EGTSCDSDGLNode:
    JSON_FILE_PATH = 'options.json'
    CATEGORY_KEYS = ['Light perception', 'lighting']
    ...
    def get_input_types_from_keys(keys):
        input_types = {}
        for key in keys:
            input_types[key] = (tuple(EGTSCDSDGLNode.get_options_keys(key)), {"default": "None"})
    ...
    def get_options_keys(key):
        ...
        parent_dir = os.path.abspath(os.path.join(current_dir, '..'))
        json_dir = os.path.join(parent_dir, 'json')  
        json_file_path = os.path.join(json_dir, EGTSCDSDGLNode.JSON_FILE_PATH)
...
NODE_CLASS_MAPPINGS = { "EG_TSCDS_DG" : EGTSCDSDGLNode }
NODE_DISPLAY_NAME_MAPPINGS = { "EG_TSCDS_DG" : "2🐕Lighting Class" }
