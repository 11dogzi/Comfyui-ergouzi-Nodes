...
class EGTSCDSJTLNode:
    JSON_FILE_PATH = 'options.json'
    CATEGORY_KEYS = ['Perspective', 'Positioning', 'Action', 'Composition Method', 'Character Lens','Lens', 'Camera Lens']
    ...
    def get_input_types_from_keys(keys):
        input_types = {}
        for key in keys:
            input_types[key] = (tuple(EGTSCDSJTLNode.get_options_keys(key)), {"default": "None"})
    ...
    def get_options_keys(key):
        ...
        parent_dir = os.path.abspath(os.path.join(current_dir, '..'))
        json_dir = os.path.join(parent_dir, 'json')  
        json_file_path = os.path.join(json_dir, EGTSCDSJTLNode.JSON_FILE_PATH)
...
NODE_CLASS_MAPPINGS = { "EG_TSCDS_JT" : EGTSCDSJTLNode }
NODE_DISPLAY_NAME_MAPPINGS = { "EG_TSCDS_JT" : "2🐕Lens class" }
